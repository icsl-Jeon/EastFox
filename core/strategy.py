    ...
    def __apply_selector_sequence(self, selector_sequence: SelectorSequence, query_date: datetime.date) -> set[str]:
        ...
        for selector in selector_sequence.selectors:
            ...
            if selector.type == SelectorType.PROFILE:
                ...
                df_profile: pd.DataFrame | None = self._fetcher.get_history_list(selector.key,
                                                                                 symbol_list=asset_pool,
                                                                                 from_date=query_date - timedelta(
                                                                                     365 * selector.horizon),
                                                                                 to_date=query_date)

                if df_profile is None:
                    print(
                        f"Could not get historical table {selector.key} over {query_date - timedelta(365 * selector.horizon)} to {query_date} !")
                    return set()
                mean_table_asset_pool: pd.Series = df_profile.mean()

                if not selector.is_absolute:
                    mean_table_asset_pool = mean_table_asset_pool.rank(pct=True)

                asset_pool = mean_table_asset_pool[
